...
#[derive(Debug, Clone)]
pub struct PublicKeyList(Vec<unc_crypto::PublicKey>);
...
impl std::str::FromStr for PublicKeyList {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let keys: Vec<unc_crypto::PublicKey> = s
            .split(',')
            .map(|str| str.trim().parse())
            .collect::<Result<Vec<unc_crypto::PublicKey>, _>>()?;
        Ok(Self(keys))
    }
}

impl From<PublicKeyList> for Vec<unc_crypto::PublicKey> {
    fn from(item: PublicKeyList) -> Self {
        item.0
    }
}

impl From<Vec<unc_crypto::PublicKey>> for PublicKeyList {
    fn from(item: Vec<unc_crypto::PublicKey>) -> Self {
        Self(item)
    }
}

...
pub(crate) fn generate_abi(
    ...
    let unc_sdk_dep = root_node
        .deps
        .iter()
        .find(|dep| dep.name == "unc_sdk")
        .and_then(|unc_sdk| {
            crate_metadata
                .raw_metadata
                .packages
                .iter()
                .find(|pkg| pkg.id == unc_sdk.pkg)
        })
        .wrap_err("`unc-sdk` dependency not found")?;

    for required_feature in ["__abi-generate", "__abi-embed"] {
        if !unc_sdk_dep.features.contains_key(required_feature) {
            color_eyre::eyre::bail!("unsupported `unc-sdk` version. expected 4.1.* or higher");
        }
    }

    let unc_sdk_metadata = crate_metadata
        .root_package
        .dependencies
        .iter()
        .find(|dep| dep.name == "unc-sdk")
        .wrap_err("`unc-sdk` dependency not found")?;

    // `Dependency::features` return value does not contain default features, so we have to check
    // for default features separately.
    if !unc_sdk_metadata.uses_default_features
        && !unc_sdk_metadata
            .features
            .iter()
            .any(|feature| feature == "abi")
    {
        color_eyre::eyre::bail!("`unc-sdk` dependency must have the `abi` feature enabled")
    }
    ...
    let dylib_artifact = util::compile_project(
        ...
        &[
            &["--features", "unc-sdk/__abi-generate"] as &[&str],
            cargo_args,
        ]
        .concat(),
        vec![
    ...
    let mut contract_abi = util::handle_step("Extracting ABI...", || {
        ...
        Ok(unc_abi::__private::ChunkedAbiEntry::combine(abi_entries)?
...
pub(crate) fn write_to_file(
    ...
    let unc_abi_serialized = match format {
        AbiFormat::Json => serde_json::to_vec_pretty(&contract_abi)?,
        AbiFormat::JsonMin => serde_json::to_vec(&contract_abi)?,
    };
    let unc_abi_compressed = match compression {
        AbiCompression::NoOp => unc_abi_serialized,
        AbiCompression::Zstd => zstd::encode_all(
            unc_abi_serialized.as_slice(),
    ...
    fs::write(&out_path_abi, unc_abi_compressed)?;
...
fn extract_metadata(crate_metadata: &CrateMetadata) -> unc_abi::AbiMetadata {
...
fn strip_docs(abi_root: &mut unc_abi::AbiRoot) {
...
pub fn run(args: super::AbiCommand) -> crate::CliResult {

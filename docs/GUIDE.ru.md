...
Это руководство предназначено для того, чтобы дать подробное описание утилиты _unc CLI_ и
обзор её возможностей. Предполагается, что утилита _unc CLI_
...
Спомощью _unc CLI_ можно создать, подписать и отправить транзакцию в режиме _online_, который включен по умолчанию.
...
1. Подпись транзакции
   ...
   _unc CLI_ предполагает несколько способов подписи созданной транзакции. Рассмотрим подробнее каждый.

    - _sign-with-keychain - Sign the transaction with a key saved in the secure keychain_

        _unc CLI_ позволяет хранить и извлекать пароли базовом безопасном хранилище, специфичном для конкретной ОС. В этом хранилище _unc CLI_ самостоятельно найдет ключи доступа и подпишет созданную транзакцию.

    - _sign-with-legacy-keychain - Sign the transaction with a key saved in legacy keychain (compatible with the old unc CLI)_

        _unc CLI_ самостоятельно найдет ключи доступа и подпишет созданную транзакцию.
        Каталог с ключами доступа определен в [конфигурационном файле](#config---manage-connections-in-a-configuration-file).
        Ключи доступа должны находиться в файле _публичный-ключ.json_, расположенном в _/Users/user/.unc-credentials/имя-сети/имя-пользователя/_.
        Например, _/Users/frovolod/.unc-credentials/testnet/volodymyr.testnet/ed25519_8h7kFK4quSUJRkUwo3LLiK83sraEm2jnQTECuZhWu8HC.json_
    ...
    - _sign-with-plaintext-private-key - Sign the transaction with a plaintext private key_
        ...
        При выборе этого варианта подписи _unc CLI_ попросит пользователя ввести ключи доступа:
    ...
    - _sign-with-access-key-file - Sign the transaction using the account access key file (access-key-file.json)_
        ...
        При выборе этого варианта подписи _unc CLI_ попросит пользователя ввести путь к файлу, в котором находится информация о ключах доступа к аккаунту.

    - _sign-with-seed-phrase - Sign the transaction using the seed phrase_

        При выборе этого варианта подписи _unc CLI_ попросит пользователя ввести мнемоническую фразу, связанную с аккаунтом.
...
По умолчанию - это https://app.myuncwallet.com/ (для testnet - https://testnet.myuncwallet.com/). Но вы можете изменить адрес для авторизации с помощью флага `--wallet-url`:
```txt
unc account \
    import-account \
    using-web-wallet \
...
После успешной авторизации в _[unc Wallet](https://wallet.unc.org/)_ необходимо вернуться в терминал и ввести имя пользователя.
<details><summary><i>Результат выполнения команды</i></summary>
...
С помощью этой команды можно создать как суб-аккаунт, так и аккаунт с коротким именем, например, alice.unc или alice.testnet (в сети testnet).
Ключи доступа к создаваемому аккаунту можно добавить несколькими способами:
- [autogenerate-new-keypair](#autogenerate-new-keypair---Automatically-generate-a-key-pair)
- [use-manually-provided-seed-prase](#use-manually-provided-seed-prase---Use-the-provided-seed-phrase-manually)
- [use-manually-provided-public-key](#use-manually-provided-public-key---Use-the-provided-public-key-manually)
...
Данная команда служит для перевода средств unc токенах между аккаунтами. Обратите внимание, что количество пересылаемых токенов указывается совместно с размерной единицей (это unc либо yoctounc).
Для выполнения этой команды необходимо ввести в командной строке терминала:
```txt
unc tokens \
    fro_volod.testnet \
...
Для просмотра средств в unc токенах на счету аккаунта необходимо ввести в командной строке терминала:
```txt
unc tokens \
    fro_volod.testnet \
    view-unc-balance \
...
Чтобы поручить ваши unc токены в общий фонд одного из валидаторов и получения наград, передайте ваши unc токены и переведите их в ставку валидатора, используя следующую командну в терминале (обратите внимание, что вам необходимо использовать свой собственный идентификатор аккаунта, указать желаемое количество unc токенов, а также выбрать идентификатор аккаунта валидатора):
...
```txt
<volodymyr.testnet> has successfully deposited and staked 15 unc on <aurora.pool.f863973.m0>.
```
</details>
...
Чтобы поручить ваши unc токены в общий фонд одного из валидаторов и получения наград, переведите раннее переданные unc токены в ставку валидатора, используя следующую командну в терминале (обратите внимание, что вам необходимо использовать свой собственный идентификатор аккаунта, указать желаемое количество unc токенов, а также выбрать идентификатор аккаунта валидатора):
...
```txt
<volodymyr.testnet> has successfully stake 5 unc on <aurora.pool.f863973.m0>.
```
</details>
...
Чтобы поручить ваши unc токены в общий фонд одного из валидаторов и получения наград, переведите все раннее переданные unc токены в ставку валидатора, используя следующую командну в терминале (обратите внимание, что вам необходимо использовать свой собственный идентификатор аккаунта, а также выбрать идентификатор аккаунта валидатора):
```txt
unc staking \
    delegation volodymyr.testnet \
...
Чтобы отменить определённую часть ставки, совершённую через общий фонд валидатора, используйте следующую командну в терминале (обратите внимание, что вам необходимо использовать свой собственный идентификатор аккаунта, указать желаемое количество unc токенов, а также выбрать идентификатор аккаунта валидатора):
...
```txt
<volodymyr.testnet> has successfully unstake 7 unc from <aurora.pool.f863973.m0>.
```
</details>
...
Чтобы получить ваши раннее порученные unc токены и награды из общего фонда валидатора после того как ставка была снята и прошло 4 эпохи, используйте следующую командну в терминале (обратите внимание, что вам необходимо использовать свой собственный идентификатор аккаунта, указать желаемое количество unc токенов, а также выбрать идентификатор аккаунта валидатора):
...
Чтобы получить все ваши раннее порученные unc токены и награды из общего фонда валидатора после того как ставка была снята и прошло 4 эпохи, используйте следующую командну в терминале (обратите внимание, что вам необходимо использовать свой собственный идентификатор аккаунта, а также выбрать идентификатор аккаунта валидатора):
